...
object MushroomMining1 {
  def main(args : Array[String]){
    val sparkConf = new SparkConf().setAppName("MushroomClassify")
    val sc = new SparkContext(sparkConf)
    //Read data from local
    val mushroom = sc.textFile("s3://sunxiaohua-sentimentjob/mushroom/")
    ...
    NBPredictionAndLabel.take(100).foreach(println)
